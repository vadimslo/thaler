    ...
    <>
      ...
      <NumSection n="02" label="Find your entry" title="Two doors.">
        <div className="grid gap-px bg-line sm:grid-cols-2">
          <Door href="/app/swap/" title="Hold the currency" body="Buy and sell THALER on the canonical pool. Supply is capped; the tax on every trade feeds the reserves." cta="Buy THALER" />
          <Door href="/app/banks/" title="Operate a bank" body="A charter is a licence to issue. Open branches, collect issuance every second, exit when you choose." cta="Acquire a charter" primary />
        </div>
      </NumSection>

      <NumSection n="03" label="Open for inspection" title="See the system as it stands.">
        <div className="grid gap-px bg-line md:grid-cols-[minmax(0,1fr)_320px]">
          <div className="min-w-0 bg-ink-2 p-5 sm:p-6">
            ...
            <Link href="/app/protocol/contracts/" className="link-quiet mt-4 inline-block text-sm">Addresses, pool id, how to verify <Icon name="arrow" size={12} className="ml-1 inline" /></Link>
          </div>
          <div className="min-w-0 bg-ink-2 p-5 sm:p-6">
            ...
            <Link href="/app/whitepaper/" className="btn mt-5 w-full">Read the whitepaper</Link>
            <Link href="/app/protocol/params/" className="link-quiet mt-3 block text-sm">Every parameter, live <Icon name="arrow" size={12} className="ml-1 inline" /></Link>
            <Link href="/app/updates/" className="link-quiet mt-2 block text-sm">Changelog <Icon name="arrow" size={12} className="ml-1 inline" /></Link>
          </div>
        </div>
      </NumSection>
    </>
...
function Door({ href, title, body, cta, primary }: { href: string; title: string; body: string; cta: string; primary?: boolean }) {
  return (
    <Link href={href} className="group block bg-ink-2 p-6 transition-colors hover:bg-ink-3 sm:p-8">
      ...
      <span className={`btn btn-sm mt-6 ${primary ? "btn-primary" : ""}`}>{cta} <Icon name="arrow" size={12} /></span>
    </Link>
  );
}
