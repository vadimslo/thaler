...
/**
 * Newest protocol events straight from the chain. `limit` rows are shown out of `fetchLimit` read
 * (default 20); `types` keeps only those event types; `footer` renders under the table. `compact`
 * keeps the two-line list at every width (for narrow containers such as the ledger drawer).
 */
export function ActivityFeed({ limit = FEED_LIMIT, fetchLimit = FEED_LIMIT, title = "Activity", types, footer, compact = false }: { limit?: number; fetchLimit?: number; title?: string; types?: string[]; footer?: React.ReactNode; compact?: boolean }) {
  const a = useActivity(fetchLimit);
  const all = a.data?.rows ?? [];
  const filtered = types ? all.filter((r) => types.includes(r.type)) : all;
  const rows = filtered.slice(0, limit);
  const paused = a.isError;
  const scanning = a.data?.backfilling;
  const empty = a.isLoading ? "Reading the first window of logs." : paused ? "No rows cached yet. The feed resumes when the RPC answers." : types && all.length > 0 ? "No events of this kind in the rows read so far." : "No events since deployment.";
  ...
  return (
    <div className="card rise">
      ...
      {/* Mobile and compact: two lines per event. */}
      <ul className={compact ? "" : "sm:hidden"}>
        {rows.length === 0 ? (
          <li className="px-4 py-3 text-xs text-paper-3">{empty}</li>
        ) : (
          rows.map((r) => (
        ...
        <div className="hidden overflow-x-auto sm:block">
          <table className="feed">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={4} className="text-paper-3">{empty}</td>
                </tr>
              ) : (
                rows.map((r) => (
                  <tr key={r.id}>
      ...
      {footer && <div className="hairline-t px-4 py-2.5 text-xs sm:px-5">{footer}</div>}
    </div>
  );
}
