...
import { fmtBps, fmtClock, fmtDuration, fmtEth, fmtInt, fmtNum, fmtNumEth, fmtToken, n18, NA, share } from "@/lib/format";
import { AuctionCurve } from "./AuctionCurve";
import { Count } from "./Count";
import { DeploymentPending, Muted, NumSection, Progress, TxStatus } from "./ui";

export function ChartersLive() {
  if (!deployed) {
    return (
...
function Inner() {
  ...
  const canAct = isConnected && onSepolia;
  const p = useParams();
  ...
  const refetchSale = () => { sale.refetch(); policy.refetch(); mine.refetch(); };
  const refetchMine = () => { details.refetch(); policy.refetch(); wallet.refetch(); mine.refetch(); };

  return (
    <>
      <div className="grid gap-px bg-line lg:grid-cols-2">
        <FoundingCard p={p} sale={sale} canAct={canAct} isConnected={isConnected} onDone={refetchSale} />
        <AuctionCard sale={sale} canAct={canAct} isConnected={isConnected} onDone={refetchSale} />
      </div>

      <NumSection
        n="02"
        label="My charters"
        title={address ? undefined : "Connect a wallet."}
      ...
      >
        {!isConnected && ids.length === 0 ? (
          <Muted>Connect a wallet to see the charters it holds.</Muted>
          ...
          <Muted>No charters in this wallet. Mint one above.</Muted>
        ) : (
          <div className="grid gap-4 lg:grid-cols-2">
            {details.rows.map((row) => (
              <CharterCard key={row.id.toString()} row={row} policy={policy} p={p} wallet={wallet} canAct={canAct} polledAt={details.dataUpdatedAt} onDone={refetchMine} />
            ))}
          </div>
        )}
      </NumSection>
...
type SaleT = ReturnType<typeof useCharterSale>;
type ParamsT = ReturnType<typeof useParams>;
type PolicyT = ReturnType<typeof usePolicy>;
type WalletT = ReturnType<typeof useWallet>;

function FoundingCard({ p, sale, canAct, isConnected, onDone }: { p: ParamsT; sale: SaleT; canAct: boolean; isConnected: boolean; onDone: () => void }) {
    ...
    <div className="rise bg-ink-2 p-5 sm:p-6">
      ...
      <div className="mt-6 flex items-stretch gap-2">
        ...
        <button className="btn btn-primary flex-1" disabled={!canAct || !known || soldOut || capped || total === undefined || tx.busy} onClick={mint}>
          {tx.busy ? "Minting" : soldOut ? "Sold out" : !isConnected ? "Connect to mint" : !known ? "Mint" : capped ? "Wallet cap reached" : `Mint ${q} for ${fmtEth(total)} ETH`}
        </button>
      </div>
...
function AuctionCard({ sale, canAct, isConnected, onDone }: { sale: SaleT; canAct: boolean; isConnected: boolean; onDone: () => void }) {
  const tx = useTx(onDone);
  const now = useNow();
    ...
    <div className="rise bg-ink-2 p-5 sm:p-6">
      ...
      <div className="mt-4">
        <button className="btn btn-primary w-full" disabled={!canAct || none || sale.auctionPrice === undefined || tx.busy} onClick={buy}>
          {tx.busy ? "Buying" : none ? "None left today" : !isConnected ? "Connect to buy" : sale.auctionPrice === undefined ? "Buy one" : `Buy one at ${fmtEth(sale.auctionPrice, 5)} ETH`}
        </button>
      </div>
      <TxStatus phase={tx.phase} hash={tx.hash} error={tx.error} onReset={tx.reset} successText="Bought." />
      <div className="mt-4"><Muted>Opens each day at three times the last close and halves every four hours toward the floor. Excess ETH is refunded. Price refreshes every 10 seconds.</Muted></div>
    </div>
  );
}
...
function CharterCard({ row, policy, p, wallet, canAct, polledAt, onDone }: { row: CharterDetail; policy: PolicyT; p: ParamsT; wallet: WalletT; canAct: boolean; polledAt: number; onDone: () => void }) {
  ...
  const needsApproval = price !== undefined && (wallet.bankAllowance ?? 0n) < price;
  const cantAfford = price !== undefined && wallet.thalerBalance !== undefined && wallet.thalerBalance < price;
    ...
    <div className="card rise p-5 sm:p-6">
      ...
      {gone ? (
        ...
        <div className="mt-5 grid gap-2 sm:grid-cols-3">
          <div>
            <button className="btn btn-primary w-full" disabled={!canAct || busy || !row.pending || row.pending === 0n} onClick={withdraw}>
          ...
          <div>
            {needsApproval && !maxed && !noLicenses ? (
              <button className="btn w-full" disabled={!canAct || busy || cantAfford || price === undefined} onClick={approve} title={`Approve ${fmtToken(price)} THALER to the central bank`}>
                {approveTx.busy ? "Approving" : cantAfford ? "Not enough THALER" : `Approve ${fmtToken(price, 0)}`}
              </button>
            ) : (
              <button className="btn w-full" disabled={!canAct || busy || maxed || noLicenses || cantAfford || price === undefined} onClick={open}>
          ...
          <div>
            {confirmResolve ? (
              ...
              <button className="btn btn-danger w-full" disabled={!canAct || busy} onClick={() => setConfirmResolve(true)}>
